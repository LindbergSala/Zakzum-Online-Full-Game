...
Static quest definitions now exist in `lib/game/questData.js`. A protected character quest API lists duties at an owned character's current location and can accept an available quest. The Quest section remains read-only. `CharacterQuest` provides per-character status persistence for static quest keys, while completion, rewards, combat, and story progression have not been added.
