...
Static quest definitions now exist in `lib/game/questData.js`. A protected read-only character quest API and read-only Quest section return the static duties available at an owned character's current location. `CharacterQuest` provides per-character status persistence for static quest keys, while acceptance, completion, rewards, combat, and story progression have not been added.
...
The Character model foundation, protected character API routes, dashboard character creation UI, character detail page, inventory and equipment foundations, ActivityLog persistence and automatic logs, static world location data, protected travel and rest flows, static quest data, and CharacterQuest persistence now exist. Quest acceptance, completion, rewards, combat, shops, map UI, and story progression have not been added yet.
...
1. Add protected quest acceptance behavior before quest completion or rewards.
