...
Static quest definitions now exist in `lib/game/questData.js`. Protected character quest APIs list duties, merge progress, accept available quests, and complete accepted quests. The Quest page can accept available quests, complete accepted quests, and display persisted status. `CharacterQuest` provides per-character status persistence for static quest keys. Static gold, experience, and renown reward definitions and validation rules now exist, but rewards are not applied. Combat and story progression have not been added.
...
The Character model foundation, protected character API routes, dashboard character creation UI, split character pages, inventory and equipment foundations, ActivityLog persistence and automatic logs, static world location data, protected travel and rest flows, static quest data, CharacterQuest persistence, quest acceptance API and UI, completion rules, protected quest completion API and UI, and static quest reward rules now exist. Reward application, combat, shops, map UI, and story progression have not been added yet.
...
1. Apply validated quest rewards atomically during completion without adding item rewards.
