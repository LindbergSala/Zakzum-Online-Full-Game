...
Static quest definitions now exist in `lib/game/questData.js`. Protected character quest APIs list duties, merge quest and objective progress, accept available quests, complete valid objectives, and complete accepted quests. The Quest page can accept available quests, complete objectives for accepted quests, complete accepted quests after required objectives are complete, display persisted status, preview rewards, and show completion reward and level feedback. `CharacterQuest` provides per-character status persistence for static quest keys, and `CharacterQuestObjective` stores per-objective progress keyed to static objective definitions. Protected quest reads safely merge that progress with static objective text. Validated gold, experience, and renown rewards are applied atomically during quest completion after required objectives are complete. Experience rewards can increase `Character.level` through the level progression rules in `lib/game/levelRules.js`, and the character overview shows read-only level progress. Stat increases and `maxStamina` increases have not been added. Static objectives use explicit stable keys, and objective normalization, validation, and completion-check helpers exist. Item rewards, combat, and story progression have not been added.
...
The Character model foundation, protected character API routes, dashboard character creation UI, split character pages, inventory and equipment foundations, ActivityLog persistence and automatic logs, static world location data, protected travel and rest flows, static quest data, CharacterQuest and CharacterQuestObjective persistence, quest acceptance API and UI, completion and objective rules, protected objective and quest completion APIs, quest completion UI, atomic quest reward application, read-only reward feedback, read-only level progress feedback, and quest-completion level progression now exist. Manual level-up UI, stat increases, item rewards, combat, shops, map UI, and story progression have not been added yet.
...
1. Add item rewards or stat increases only after the current quest completion and level feedback flows stay stable.
